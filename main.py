...
DEFAULT_QUALITY = (
    "resolution=640x480 crf=32 audio_bitrate=60k fps=18 preset=ultrafast codec=libx265"
)
USERS = []
ADMINS = []
SUPER_ADMINS = []
...
async def send_to_super_admin(message: Message, reason: str):
    for super_admin in SUPER_ADMINS:
        try:
            ...
            logging.info(f"No se pudo enviar alerta al administrador supremo {super_admin}.")
    ...
    if message.from_user.id in USERS:
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("Ir al canal 📣", url=f"https://t.me/{CHANNEL_USERNAME}"),
                        InlineKeyboardButton("Contactar al administrador 👨‍💼", url=f"https://t.me/{ADMIN_USERNAME}")
                    ]
                ]
            ),
        )
        ...
        if user.startswith("@"):
            try:
                ...
                if user_id not in USERS:
                    ...
                    await message.reply_text(f"¡Usuario @{user} agregado correctamente! ✅")
                else:
                    await message.reply_text(f"¡El usuario @{user} ya está en la lista! 🤔")
            except Unauthorized:
                await message.reply_text("¡No tengo acceso a este usuario! 🚫")
        else:
            await message.reply_text("¡Por favor, ingresa un usuario válido con el formato @username! ✍️")
    else:
        ...
        if user.startswith("@"):
            try:
                ...
                if user_id in USERS:
                    ...
                    await message.reply_text(f"¡Usuario @{user} baneado correctamente! 🚫")
                else:
                    await message.reply_text(f"¡El usuario @{user} no está en la lista! 🤔")
            except Unauthorized:
                await message.reply_text("¡No tengo acceso a este usuario! 🚫")
        else:
            await message.reply_text("¡Por favor, ingresa un usuario válido con el formato @username! ✍️")
        ...
        if user.startswith("@"):
            try:
                ...
                if user_id not in ADMINS:
                    ...
                    await message.reply_text(f"¡Administrador @{user} agregado correctamente! ✅")
                else:
                    await message.reply_text(f"¡El administrador @{user} ya está en la lista! 🤔")
            except Unauthorized:
                await message.reply_text("¡No tengo acceso a este usuario! 🚫")
        else:
            await message.reply_text("¡Por favor, ingresa un usuario válido con el formato @username! ✍️")
        ...
        if user.startswith("@"):
            try:
                ...
                if user_id in ADMINS:
                    ...
                    await message.reply_text(f"¡Administrador @{user} baneado correctamente! 🚫")
                else:
                    await message.reply_text(f"¡El administrador @{user} no está en la lista! 🤔")
            except Unauthorized:
                await message.reply_text("¡No tengo acceso a este usuario! 🚫")
        else:
            await message.reply_text("¡Por favor, ingresa un usuario válido con el formato @username! ✍️")
...
async def info(client: Client, message: Message):
    if len(message.command) > 1:
        ...
        await message.reply_text("¡Comando inválido! ❌ ¡Ingresa un mensaje después del comando /info!")
    ...
    if len(message.command) == 2:
        try:
            ...
            if size > 0:
                ...
                await message.reply_text(f"¡Límite de tamaño de video actualizado a {size} MB! ✅")
            else:
...
async def calidad(client: Client, message: Message):
    ...
    if message.from_user.id in USERS:
        if len(message.command) == 2:
            ...
            await message.reply_text("¡Comando inválido! ❌ ¡Ingresa la calidad después del comando /calidad!")
    else:
        await send_to_super_admin(
            message,
            f"**Alerta:** Usuario no autorizado intentó usar un comando de administrador.",
...
async def listuser(client: Client, message: Message):
    if USERS:
        user_list = ", ".join([f"@{await app.get_users(user_id).username}" for user_id in USERS])
...
async def handle_video_private(client: Client, message: Message):
    ...
    try:
        ...
        # Comando para la conversión
        cmd = (
            f'ffmpeg -i "{new_video}" -map 0 -c:v libx265 -preset ultrafast -crf 23 -c:a aac -b:a 128k -map 0 -movflags faststart -y "{new_video.replace(".mp4", "_converted.mp4")}"'
        )
        os.system(cmd)
        ...
        await app.send_video(
            ...
            caption=f"꧁༺ 𝑭𝒊𝒏𝒂𝒍𝒊𝒛𝒂𝒅𝒐 ༻꧂\n"
                   ...
                   f"×͜×𝑷𝒆𝒔𝒐 𝑷𝒓𝒐𝒄𝒆𝒔𝒂𝒅𝒐: {os.path.getsize(f'{new_video.replace(".mp4", "_converted.mp4")}') // (1024 * 1024)} MB\n"
...
async def handle_video_thumbnail_private(client: Client, message: Message):
    ...
    try:
        ...
        video = await app.download_media(message.video)

        # Redimensiona la imagen a 90x90
        ...
        # Comando para la conversión
        cmd = (
            f'ffmpeg -i "{video}" -i "{thumbnail}" -map 0 -c:v libx265 -preset ultrafast -crf 23 -c:a aac -b:a 128k -map 0 -movflags faststart -map 1 -c:v copy -disposition:v 0 -y "{video.replace(".mp4", "_converted.mp4")}"'
        )
        os.system(cmd)
        ...
        # Envía el video convertido
        await app.send_video(
            chat_id=message.chat.id,
            video=f"{video.replace('.mp4', '')}_converted.mp4",
            thumb=thumbnail,
            caption=f"꧁༺ 𝑭𝒊𝒏𝒂𝒍𝒊𝒛𝒂𝒅𝒐 ༻꧂\n"
                   f"✮𝑵𝒐𝒎𝒃𝒓𝒆 𝒅𝒆𝒍 𝒗𝒊𝒅𝒆𝒐 {message.video.file_name}\n"
                   f"×͡×𝑷𝒆𝒔𝒐 𝑶𝒓𝒊𝒈𝒊𝒏𝒂𝒍: {message.video.file_size // (1024 * 1024)} MB\n"
                   f"×͜×𝑷𝒆𝒔𝒐 𝑷𝒓𝒐𝒄𝒆𝒔𝒂𝒅𝒐: {os.path.getsize(f'{video.replace(".mp4", "_converted.mp4")}') // (1024 * 1024)} MB\n"
                   f"𖤍𝑻𝒊𝒆𝒎𝒑𝒐 𝒅𝒆 𝑷𝒓𝒐𝒄𝒆𝒔𝒂𝒎𝒊𝒆𝒏𝒕𝒐: {conversion_time_formatted}\n"
                   f"✧𝑻𝒊𝒆𝒎𝒑𝒐 𝒅𝒆𝒍 𝑽𝒊𝒅𝒆𝒐: {video_duration}\n"
                   f"♣¡𝑸𝒖𝒆 𝒍𝒐 𝒅𝒊𝒔𝒇𝒓𝒖𝒕𝒆𝒔 ✨",
        )

        # Elimina los archivos originales y el convertido
        os.remove(thumbnail)
        os.remove(video)
        os.remove(f"{video.replace('.mp4', '')}_converted.mp4")
