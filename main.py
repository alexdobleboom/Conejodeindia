...
USER_ADMIN = "TheDemonsuprem" # Reemplázalo con el nombre de usuario del administrador supremo
...
def notify_admins(message):
    ...
    conn.close()
    for admin in admins:
        app.send_message(chat_id=admin[0], text=message)
...
async def add_admin_command(client, message: Message):
    ...
    if is_admin(username) and username == USER_ADMIN:
        args = message.text.split()
        if len(args) == 2:
            new_admin = args[1]
            add_admin(new_admin)
            app.send_message(chat_id=message.chat.id, text=f"El usuario @{new_admin} ha sido agregado como administrador.")
        else:
