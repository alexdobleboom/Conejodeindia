...
admin_users = ["@Thedemonsuprem", "@Sasuke286"] # Reemplaza con los usernames de tus administradores
...
async def compress_video(client, message: Message, quality_settings=None):
    ...
    if message.reply_to_message and message.reply_to_message.video:
        ...
        await client.send_message(chat_id=message.chat.id, text="🔄 ¡Comprimiendo el video! 🔄")
        ...
        try:
            ...
            await client.send_message(chat_id=message.chat.id, text=f"❌ ¡Error al comprimir el video! {e} ❌")
        finally:
...
async def error_handler(client, exception, message: Message):
    await client.send_message(chat_id=message.chat.id, text=f"❌ ¡Error! {exception} ❌")

# Comando /start
@app.on_message(filters.command("start"))
async def start_handler(client, message: Message):
    await client.send_message(chat_id=message.chat.id, text="🤖 ¡Hola! 👋 ¿Qué quieres hacer? 🤔")
...
async def about_handler(client, message: Message):
    await client.send_message(chat_id=message.chat.id, text="Soy un bot de compresión creado por @TheDemonsuprem, versión del bot v2.0 🤖.")
