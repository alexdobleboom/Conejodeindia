...
admin_users = ["@TheDemonsuprem", "@TheBigBoss1ro"] # Reemplaza con los usernames de tus administradores
...
async def error_handler(client, exception, message: Message):
    if isinstance(exception, (FloodWait, TimeoutError)):
        ...
        await asyncio.sleep(60)
                                ...
                                UserPrivacyRestrictedError,
                                MessageNotModified, MessageTooLong, RPCError)):
...
async def about_handler(client, message: Message):
    await client.send_message(chat_id=message.chat.id, text="🤖 Soy un bot de compresión de video. Creado por The Big Boss. Versión: 2.0 🤖")
