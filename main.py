...
users = ['@TheDemonsuprem', '@Sasuke286']
admins = ['@TheDemonsuprem', '@Sasuke286']
...
about_message = """
Soy un bot creado por [TU NOMBRE] para comprimir videos.
...
async def compress_video(client, message, quality):
    try:
...
async def add_user(client, message, username):
    try:
        # Validar usuarios
...
async def ban_user(client, message, username):
    try:
        # Validar usuarios
        if not await validate_user(client, message, admins + super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
...
async def add_admin(client, message, username):
    try:
        # Validar usuarios
        if not await validate_user(client, message, admins + super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
...
async def ban_admin(client, message, username):
    try:
        # Validar usuarios
...
async def send_info(client, message, info_text):
    try:
        # Validar usuarios
        if not await validate_user(client, message, super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
...
async def set_max_size(client, message, new_max_size):
    try:
        # Validar usuarios
        if not await validate_user(client, message, admins + super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
...
async def set_quality(client, message, new_quality):
    try:
        # Validar usuarios
...
async def add_user_command(client, message):
    try:
        ...
        # Añadir el usuario
        await add_user(client, message, username)
...
async def ban_user_command(client, message):
    try:
        ...
        # Banear el usuario
        await ban_user(client, message, username)
...
async def add_admin_command(client, message):
    try:
        ...
        await add_admin(client, message, username)

    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error al agregar administrador: {e} ❌")

@app.on_message(filters.command(['banadmin']))
async def ban_admin_command(client, message):
    try:
        # Obtener el nombre de usuario
        username = message.text.split()[1]

        # Banear el administrador
        await ban_admin(client, message, username)

    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error al banear administrador: {e} ❌")

@app.on_message(filters.command(['info']))
async def send_info_command(client, message):
    try:
        ...
        await send_info(client, message, info_text)

    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error al enviar la información: {e} ❌")

@app.on_message(filters.command(['max']))
async def set_max_size_command(client, message):
    try:
        # Validar usuarios
        if not await validate_user(client, message, admins + super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
        ...
        # Actualizar el límite del tamaño
        await set_max_size(client, message, new_max_size)

    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error al actualizar el límite del tamaño: {e} ❌")

@app.on_message(filters.command(['calidad']))
async def set_quality_command(client, message):
    try:
        # Validar usuarios
        if not await validate_user(client, message, users + admins + super_admins):
            await client.send_message(message.chat.id, "No tienes permiso para usar este comando.")
        ...
        await set_quality(client, message, new_quality)

    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error al actualizar la calidad: {e} ❌")
...
async def list_users_command(client, message):
    try:
        ...
        await list_users(client, message)
