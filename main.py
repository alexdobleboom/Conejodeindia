...
async def help_command(client, message: Message):
    ...
    * `/queue_size`: Ver el tamaño actual de la cola de compresión.
    * `/clear_queue`: Limpiar la cola de compresión.
    * `/set_queue_limit [límite]`: Establecer el límite de tamaño de la cola de compresión.
    * `/active_tasks`: Ver las tareas en curso.
...
@app.on_message(filters.command("tamcola"))
async def queue_size_command(client, message: Message):
    queue_size = await get_queue_size()
    await app.send_message(chat_id=message.chat.id, text=f"El tamaño actual de la cola es: {queue_size}")

@app.on_message(filters.command("erase"))
async def clear_queue_command(client, message: Message):
    await clear_queue()
    await app.send_message(chat_id=message.chat.id, text="Cola de compresión limpiada.")

@app.on_message(filters.command("límite"))
async def set_queue_limit_command(client, message: Message):
    global QUEUE_LIMIT
    args = message.text.split()
    if len(args) == 2:
        try:
            new_queue_limit = int(args[1])
            QUEUE_LIMIT = new_queue_limit
            await app.send_message(chat_id=message.chat.id, text=f"Límite de tamaño de la cola establecido a {new_queue_limit}.")
        except ValueError:
            await app.send_message(chat_id=message.chat.id, text="Formato inválido. Usa /set_queue_limit seguido de un número entero.")
    else:
        await app.send_message(chat_id=message.chat.id, text="Usa /set_queue_limit seguido del nuevo límite de tamaño de la cola.")

@app.on_message(filters.command("task"))
async def active_tasks_command(client, message: Message):
    if active_tasks:
        tasks_info = ""
