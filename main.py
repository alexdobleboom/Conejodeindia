...
async def split_file(chat_id, file, size):
    try:
        ...
        subprocess.run(["ffmpeg", "-i", file, "-c", "copy", "-segment_time", f"{size}", "-f", "segment", "-reset_timestamps", "1", "output_%03d.mkv"])
